...
<div  class=" p-10 bg-opacity-50 w-full flex justify-center items-center bg-cover bg-center bg-no-repeat bg-fixed"  style="background-image: url('http://127.0.0.1:8000/images/hardwareequipment.jpg');">
    
    
<article class="px-10 py-10 shadow-xl min-h-screen w-full rounded-lg bg-opacity-50 bg-white bg-cover bg-center bg-no-repeat bg-fixed" >
    ...
    @foreach($cartItems as $cartItem)
      <div class="xl:w-1/4 md:w-1/2 p-4">
        ...
        <div class="cart-item bg-white mb-4 shadow-xl grow rounded-lg">
          ...
          <input type="checkbox" name="selected_items[]" class="selected-items" value="{{ $cartItem->id }}" data-price="{{ $cartItem->product->price }}" data-id="{{ $cartItem->product->id }}"/>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    const checkboxes = document.querySelectorAll(".selected-items");
