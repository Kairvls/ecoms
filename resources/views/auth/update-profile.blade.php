...
<form action="{{ route('update.profile') }}" method="POST" class="form">
  ...
  <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6m2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0m4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4m-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10s-3.516.68-4.168 1.332c-.678.678-.83 1.418-.832 1.664z"/>
        ...
        <input placeholder="Enter your Fullname" class="input" name="fullname" type="text" value="{{ Auth::user()->fullname }}" required>
      </div>
    ...
    <div class="flex-column">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 32 32" height="20"><g data-name="Layer 3" id="Layer_3"><path d="m30.853 13.87a15 15 0 0 0 -29.729 4.082 15.1 15.1 0 0 0 12.876 12.918 15.6 15.6 0 0 0 2.016.13 14.85 14.85 0 0 0 7.715-2.145 1 1 0 1 0 -1.031-1.711 13.007 13.007 0 1 1 5.458-6.529 2.149 2.149 0 0 1 -4.158-.759v-10.856a1 1 0 0 0 -2 0v1.726a8 8 0 1 0 .2 10.325 4.135 4.135 0 0 0 7.83.274 15.2 15.2 0 0 0 .823-7.455zm-14.853 8.13a6 6 0 1 1 6-6 6.006 6.006 0 0 1 -6 6z"></path></g></svg>
        <input placeholder="Enter your Username" class="input" name="username" type="text" value="{{ Auth::user()->username }}" required>
      </div>
  ...
  <path d="M6.5 1A1.5 1.5 0 0 0 5 2.5V3H1.5A1.5 1.5 0 0 0 0 4.5v8A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5v-8A1.5 1.5 0 0 0 14.5 3H11v-.5A1.5 1.5 0 0 0 9.5 1zm0 1h3a.5.5 0 0 1 .5.5V3H6v-.5a.5.5 0 0 1 .5-.5m1.886 6.914L15 7.151V12.5a.5.5 0 0 1-.5.5h-13a.5.5 0 0 1-.5-.5V7.15l6.614 1.764a1.5 1.5 0 0 0 .772 0M1.5 4h13a.5.5 0 0 1 .5.5v1.616L8.129 7.948a.5.5 0 0 1-.258 0L1 6.116V4.5a.5.5 0 0 1 .5-.5"/>
        ...
        <input placeholder="Enter your Number" class="input" name="contact_number" type="text" value="{{ Auth::user()->contact_number }}" required>
      </div>
      <div class="flex-column">
      ...
      <div class="inputForm">
        ...
        <input placeholder="Enter your Address" class="input" name="address" type="text" value="{{ Auth::user()->address }}" required>
      </div>
      </div>
...
<script>
  document.getElementById("updateProfile").onsubmit = function (event) {
        event.preventDefault();
        let newPassword = document.getElementById("new_password").value;
        if (newPassword === 'updateProfile') {
            alert("Please confirm to cancel.");
            return;
        }
        this.action = `/update-profile`;
        this.submit();
    };
</script>
